...
public class ScoreSheetWriter {
    ...
    private String mSubstitutionImage;
    private String mTimeoutGrayImage;
    private String mTimeoutWhiteImage;
    private String mYellowCardImage;
    private String mRedCardImage;
    private String mExpulsionCardImage;
    private String mDisqualificationCardImage;
    private String mDelayWarningImage;
    private String mDelayPenaltyImage;
    ...
    public static File writeRecordedGame(Context context, RecordedGameService recordedGameService) {
        ...
        if (ContextCompat.checkSelfPermission(context, Manifest.permission.WRITE_EXTERNAL_STORAGE) == PackageManager.PERMISSION_GRANTED) {
            ...
            ScoreSheetWriter scoreSheetWriter = new ScoreSheetWriter(context, recordedGameService);
            scoreSheetWriter.init(filename);
    ...
    private ScoreSheetWriter(Context context, RecordedGameService recordedGameService) {
        mContext = context;
    ...
    private void init(String filename) {
        ...
        mSubstitutionImage = createImg(R.drawable.ic_thumb_substitution, 32, 32, 12, 12);
        mTimeoutGrayImage = createImg(R.drawable.ic_thumb_timeout, 32, 32,12,12);
        mTimeoutWhiteImage = createImg(R.drawable.ic_thumb_timeout_white, 32, 32,12,12);
        mYellowCardImage = createImg(R.drawable.yellow_card, 50, 50,16,16);
        mRedCardImage = createImg(R.drawable.red_card, 50, 50,16,16);
        mExpulsionCardImage = createImg(R.drawable.expulsion_card, 75, 50,24,16);
        mDisqualificationCardImage = createImg(R.drawable.disqualification_card, 120, 100,32,16);
        mDelayWarningImage = createImg(R.drawable.delay_warning, 50, 50,16,16);
        mDelayPenaltyImage = createImg(R.drawable.delay_penalty, 50, 50,16,16);
    }
    ...
    private void writeRecordedGameHeader() {
        ...
        formatter.setTimeZone(TimeZone.getDefault());
        infoDiv.appendChild(createCellSpan(formatter.format(new Date(mRecordedGameService.getGameSchedule())), true, false));
        ...
        Element homeTeamNameSpan = createCellSpan(mRecordedGameService.getTeamName(TeamType.HOME), true, false);
        ...
        Element guestTeamNameSpan = createCellSpan(mRecordedGameService.getTeamName(TeamType.GUEST), true, false);
    ...
    private void writeRecordedIndoorTeams() {
        if (UsageType.NORMAL.equals(mRecordedGameService.getUsageType())) {
            ...
            teamsDiv.appendChild(createTeamDiv(TeamType.HOME)).appendChild(createTeamDiv(TeamType.GUEST));
    ...
    private Element createTeamDiv(TeamType teamType) {
        ...
        for (int player: mRecordedGameService.getPlayers(teamType)) {
            teamDiv.appendChild(createPlayerSpan(teamType, player, mRecordedGameService.isLibero(teamType, player)));
        }
    ...
    private void writeRecordedStartingLineup(Element cardDiv, int setIndex) {
        if (UsageType.NORMAL.equals(mRecordedGameService.getUsageType())) {
            ...
            lineupsDiv.appendChild(createLineupDiv(TeamType.HOME, setIndex)).appendChild(createLineupDiv(TeamType.GUEST, setIndex));
    ...
    private Element createLineupDiv(TeamType teamType, int setIndex) {
        ...
        lineupDiv.appendChild(createPlayerSpan(teamType, mRecordedGameService.getPlayerAtPositionInStartingLineup(teamType, PositionType.POSITION_5, setIndex), false));
        lineupDiv.appendChild(createPlayerSpan(teamType, mRecordedGameService.getPlayerAtPositionInStartingLineup(teamType, PositionType.POSITION_6, setIndex), false));
    ...
    private String createImg(@DrawableRes int resource, int origWidthInPx, int origHeightInPx, int widthInPx, int heightInPx) {
        Element img = new Element("img");
        img.attr("src", String.format("data:image/png;base64,%s", toBase64(resource, origWidthInPx, origHeightInPx)));
        img.attr("style", String.format("width: %spx; height: %spx; ", widthInPx, heightInPx));
        return img.toString();
    }

    private String htmlSkeleton(String title) {
        int homeColor = mRecordedGameService.getTeamColor(TeamType.HOME);
        int guestColor = mRecordedGameService.getTeamColor(TeamType.GUEST);
        ...
        String homeLiberoBackgroundColor = colorIntToHtml(mRecordedGameService.getLiberoColor(TeamType.HOME));
        String homeLiberoColor = colorIntToHtml(UiUtils.getTextColor(mContext, mRecordedGameService.getLiberoColor(TeamType.HOME)));
        ...
        String guestLiberoBackgroundColor = colorIntToHtml(mRecordedGameService.getLiberoColor(TeamType.GUEST));
        String guestLiberoColor = colorIntToHtml(UiUtils.getTextColor(mContext, mRecordedGameService.getLiberoColor(TeamType.GUEST)));
